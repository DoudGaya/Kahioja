<template>
    ...
    <div class="grid grid-cols-4 gap-6">
        <div :key="productIndex.name" v-for="productIndex in productsToShow" class="px-14 mt-20 mb-8"> 
            <div class="bg-white rounded-lg px-4 my-4 cursor-pointer yus-shadow">
                <a :href="productlink">
                    ...
                    <div class="py-3">
                        ...
                        <h1 class="card-title">{{cart[productIndex].name}}</h1>
                        <div class="flex items-center">
                            <span class="card-curr-price"><b>{{cart[productIndex].price}}</b></span>
            ...
            <div>{{cart[productIndex].name}} says:</div>
            <i><div>{{cart[productIndex].price}}</div></i>
        </div>
    </div>
    <div v-if="productsToShow < cart.length || cart.length > productsToShow"  class="mt-16 mb-8 flex justify-center">
        <button  class="mx-auto btn-yus-conti-shopping rounded-full w-1/6 text-lg py-2 text-white" @click="productsToShow += 8">Shop all Items</button>
    </div>
</template>
