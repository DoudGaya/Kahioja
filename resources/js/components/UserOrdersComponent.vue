        ...
        <div id="cart-content">
            ...
            <div id="cart-body">
                ...
                <div v-if="orders.length > 0" v-show="orderList">
                    <div :key="order.id" v-for="order in orders" id="cart-body-products" class="p-4 text-center border-b-2">
                        <div class="grid grid-cols-2 gap-4 p-4 border-b items-center">
                            <div class="text-lg text-right">Order No:</div>
                            <div>{{ order.order_number}}</div>
                        </div>
                        <div class="grid grid-cols-2 gap-4 p-4 border-b items-center">
                            <div class="text-lg text-right">Payment Status:</div>
                            <div>{{ order.payment_status}}</div>
                        </div>
                        <div class="grid grid-cols-2 gap-4 p-4 border-b items-center">
                            <div class="text-lg text-right">Total Amount:</div>
                            <div>{{ order.currency_sign }}{{ order.pay_amount }}</div>
                        </div>
                        <div class="grid grid-cols-2 gap-4 p-4 border-b items-center">
                            <div class="text-lg text-right">Date:</div>
                            <div>{{ new Date(order.created_at).toLocaleString() }}</div>
                        </div>
                        <div class="my-3">
                            <input value="View Order" type="submit" @click="viewOrder(order.id, order.number)" class="flex justify-center mx-auto btn-yus rounded-full w-full py-5 lg:w-1/2 lg:py-4 text-white">
                        </div>
                     </div>
                </div>
                ...
                <div v-show="vieworder" id="vieworder" class="text-center">
...
export default {
    ...
    async created(){
        ...
        axios.get(`/orders`).then(response => {
            ...
            this.bags = response.data.bags;
        }).catch(error => {
            console.log(error)
        })
    },
    methods:{
        viewOrder(order_id, title, currency, price, days, products){
