...
class CartController extends Controller
{
    public function index(Request $request)
    {
        ...
        $bag = DB::select("SELECT DISTINCT bags.id as 'bagId', bags.quantity, products.name, products.price, bags.ship_fee, products.photo, products.slug, bags.amount as 'subTotal' FROM bags, products, users WHERE bags.product_id = products.id && bags.user_id = '$user_id' && bags.paid = 'unpaid' ORDER BY bags.id DESC");
    ...
    public function addtobag(Request $request)
    {
        ...
        $product_moq = Product::select('minimum_order_qty')->where('id', $product_id)->pluck('minimum_order_qty')->first();

        if($product_moq != ''){
            if($quantity < $product_moq){
                $quantity = $quantity + $product_moq;
            }
        }
        ...
        $is_product_in_bag = Bag::where('product_id', $product_id)->where('user_id', $user_id)->where('paid', 'unpaid')->where('vendor_id', $vendor_id)->count();
            if($is_product_in_bag == 0){
                ...
                if($request->quantity == 1){
                    $quantity = $quantity_check[0]->quantity + 1;
                }
                ...
                $bag_id = $quantity_check[0]->id;
                    ...
                    try{
                        ...
                        $bag = DB::select("SELECT DISTINCT bags.id as 'bagId', bags.quantity, products.name, products.price, bags.ship_fee, products.photo, products.slug, bags.amount as 'subTotal' FROM bags, products, users WHERE bags.vendor_id = '$vendor_id' &&  bags.product_id = products.id && bags.user_id = '$user_id' && bags.paid = 'unpaid' ORDER BY bags.id DESC");
                        return $response = \Response::json($bag, 200);
